...
<Callout tone="note" label={`Permissive early, narrow later`}>
...
<Callout tone="note" label={`What subset containment costs`}>
...
<Callout tone="note" label={`The worked example is LSP, and none of it is fjord's`}>
...
<Callout tone="warn" label={`A bitmask is lost, and it is stated rather than hidden`}>
...
<Callout tone="note" label={`Two module graphs disagree on purpose`}>
...
<Callout tone="note" label={`A database that does not state its position encoding is read as \`utf16\``}>
