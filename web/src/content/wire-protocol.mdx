...
**A session bound to no database answers too**, with the catalogue alone — which is exactly
what a tool doing `create` or `list` may ask about, and all it may ask about. `h` and `y` take
that same fallback, so the two cannot disagree about what is being served.
...
<Callout tone="note" label={`A fact that interned cleanly and then conflicted stays written`}>
