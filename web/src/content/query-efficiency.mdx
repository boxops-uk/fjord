...
<Callout tone="note" label={`Why not "rows returned"`}>
...
<Callout tone="warn" label={`A join can still be a residual`}>
...
<Callout tone="note" label={`Bounding the *result* is a different question`}>
