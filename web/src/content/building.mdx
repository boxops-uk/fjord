...
└── web/                 the interactive site: this book, with the engine running in it,
                        its pages under web/src/content/
```

**This book is the design of record.** Each page is one MDX file under `web/src/content/`,
compiled into the site like any other module: prose where prose is enough, and a component
where it is not — so a diagram is drawn with the browser rather than described in dashes,
and a demo is the engine rather than a picture of its output. The smoke check counts every
block a page's source writes against the blocks the built page shows, because the way a
component mapping breaks is silent: a table whose wrapper goes missing is six paragraphs
that still read fine.

**What publishes is `web/`.** Every push to main deploys that bundle to
[https://boxops-uk.github.io/fjord/](https://boxops-uk.github.io/fjord/) — after the suite, the drift gate, and the bundle being
