...
<Callout tone="warn" label={`\`body.len()\` is not the number of loops`}>
...
<Callout tone="note" label={`The architectural rule that keeps this small`}>
...
<Callout tone="invariant" label={`I5 — a register holds the whole row`}>
...
<Callout tone="invariant" label={`I9 — the hot path is allocation-free per row`}>
...
<Callout tone="invariant" label={`I7 — the executor is a defunctionalised state machine, on purpose`}>
...
<Callout tone="note" label={`The recompute rule`}>
...
<Callout tone="note" label={`Why resume owns a battery and not a review rule`}>
