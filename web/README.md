...
| `src/HeroStory.tsx` | its hero: one find-references over the real corpus, staged. Hover, query, the seek into the index, the answer. The band is the workbench's picture at corpus scale — the stored keys around the range, shaded, with gutters for what is off each end. Everything but the pointer is engine output |
