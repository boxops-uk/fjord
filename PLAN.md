...
| Admission control for **queries** | The connection axis is built: a cap defaulting to half the soft descriptor limit, `Busy` past it, and an `accept` failure that is never fatal. What is left is the axis `bench/FINDINGS.md`'s F8 actually measured — in-flight queries queue on the blocking pool as latency, invisibly. A queue-depth limit and the wall-clock deadline in the row above are the two halves; both want the same decision about what a refused *query* costs a session |
...
| A `create` accepting a schema it cannot then serve | A schema declaring a predicate in the **reserved namespace** is written as a valid artifact by `Catalog::create` — which sees only the database's own schema — and no server can open it, because serving one composes that schema with the catalogue and marks reserved predicates virtual, which the copy the sidecar's fingerprint was taken over is not. The client is told, and told honestly (`a_create_this_server_cannot_open_is_not_an_unknown_database`, code 8), and the instance is left under the root. The fix belongs at `create`, beside the schema round-trip check already there, and not in what a failed open answers |
