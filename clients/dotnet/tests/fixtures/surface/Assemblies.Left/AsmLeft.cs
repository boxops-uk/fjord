...
// in, so that the ONLY names shared with `Assemblies.Right` are the ones shared on purpose:
// `Surface.Assemblies.Shared`'s two types (M17) and the type parameters below (M10).
namespace Surface.Assemblies.Left;
...
/// </summary>
public class AsmLeftMap<TKey, TValue>
    where TKey : notnull
{
...
/// corpus and the same key as <c>Assemblies.Right</c>'s.
/// </summary>
public interface IAsmLeftSource<out T>
{
...
/// </summary>
public static class AsmLeftUses
{
    ...
    public static AsmLeftBox<string> Boxed() => new(AsmLeftBackend.Stamp, Stamped());
}
