...
| Page | Change | From |
|---|---|---|
| **the schema set** — **landed as sections in `schema-language.md`**, not a new page | what `src`, `config`, `codemarkup` and the five reference schemas are, the layering, the "reference schemas" status sentence, the `Kind`/`Role` citation tables, and the two worked joins from `index.sigla`'s header. Sections under *The sample schema* rather than a page of their own: a new slug needs a place in the reading order, and every reader this material is for is already on the page that explains declarations. Revisit if it outgrows the page | W6, W7, W8, W9 |
| **the style layer** — **re-cut by [D13](OPEN-QUESTIONS.md), landed in `schema-language.md`** | there is no fjord format to document any more. What the book owes instead: `FileLineStyles` is `bytes`, the schema is silent about them, `config.Setting {dimension = "style-encoding"}` names format and legend together, an unrecognised value renders those lines plain, absent means "not tokenised" — and the statement that **fjord ships no codec and defines no vocabulary**. The .NET indexer's LSP payload is carried as the worked example, marked as the producer's choice rather than fjord's | W6, D13 |
